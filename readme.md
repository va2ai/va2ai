...
**How the research was run.** I treated the pipeline as a subject of study, not a design I'd guessed right the first time. The formal study plan is [`research/research_plan.md`](https://github.com/va2ai/va-claims-intel/blob/main/research/research_plan.md) — study ID `ADJ-2026-04`, *"Adjudicative Reasoning Pipeline Optimization."* It defined six research questions (retrieval vs. reasoning contribution, whether a scaffolded single-prompt can match the pipeline, breadth vs. depth trade-off, error origin, optimal model allocation, Pareto frontier), a five-phase experiment plan (measurement foundation → baseline reproduction → retrieval ablation → architecture ablation → breadth vs. depth), a 5-question benchmark spanning distinct legal categories, and a 6-dimensional scoring rubric. Assumptions and risks were documented upfront — e.g., "3 test questions may not be representative" was flagged as a Phase-1 risk, not hidden.

**The iteration history, with numbers I can defend.** Ran three architecture generations on three representative questions (sleep apnea secondary SC, rating reduction, CUE):

| Config | Question | Time | Tokens | BVA Hits | CAVC Cites | Core Correct | Actionable Next Steps |
|---|---|---|---|---|---|---|---|
| v1 6-agent | sleep apnea | 36s | 11,851 | 0 | 0 | ✓ | ✗ |
| v2 5-agent | sleep apnea | 57s | 22,867 | **24** | 4 | ✓ | ✓ |
| v3 3-agent | sleep apnea | **32s** | **11,749** | 23 | 4 | ✓ | ✓ |
| v3 3-agent | reduction | 31s | 12,592 | 24 | 5 | ✓ | ✓ |
| v3 3-agent | CUE | 35s | 12,884 | 18 | 4 | ✓ | ✓ |
| single prompt | sleep apnea | 12s | 2,834 | 0 | 2 | ✓ | ✗ |

**v1→v3 reduced the agent count from 6 to 3 at equal or better quality and ~half the token cost of v2.** The primary measurable advantage over single-prompt was evidence volume (0 → 20+ BVA hits) and actionable output. Single-prompt sometimes caught CAVC cases the pipeline missed (the "3.156(c) backdoor") — a real breadth-vs-depth trade I flagged as RQ3 rather than hiding.

**v3 architecture, with the reason each component exists.** The v3 pipeline (`scripts/test-adjudicative.ts`, productionized as the v2 research workflow at [`src/services/ai-runtime/workflows/research-v2.ts`](https://github.com/va2ai/va-claims-intel/blob/main/src/services/ai-runtime/workflows/research-v2.ts)) is a dependency graph, not a chain:

```
intake → targeted_retrieval → [authority_mapper ∥ evidence_evaluator] → conflict_detector → synthesizer → faithfulness_critic
```

Two architectural choices are load-bearing and the commentary in the code explains why:
- **The evidence evaluator runs AFTER retrieval, not in parallel.** v2 ran the evaluator in parallel on sparse intake data; it consistently conflated law with evidence. Moving it downstream so it consumes the full `EvidenceBundle` fixed the conflation. The authority mapper still runs in parallel because it only needs flat docs.
- **Targeted retrieval is hypothesis-aware.** The Framer (`gemini-3-flash-preview`) classifies the issue, identifies the governing rule, and extracts search terms before retrieval fires. Retrieval then decomposes → breadths → hypothesizes → retrieves → normalizes → ranks. Feeding the retriever a hypothesis is what drives the 0→24 BVA-hit jump over single-prompt.

**Model allocation is deliberate.** `gemini-3-flash-preview` for the Framer (fast, and it uses ~1,200 native thinking tokens anyway), `gemini-2.5-flash` with a 4,096-token thinking budget for Evaluator and Reviewer. The ablation that motivated this: `gemini-2.5-pro` thinking on CUE took 73s vs. `gemini-2.5-flash` thinking at 48s with comparable quality — pro was worth the latency only on the most doctrinally novel questions.

**Coordinator + team architecture** ([`apps/api/app/teams/coordinator.py`](https://github.com/va2ai/v2v-monorepo/blob/main/apps/api/app/teams/coordinator.py)) — six specialized agents (`intake`, `planner`, `retrieval`, `analysis`, `cite`, `refinement`) run under a `TeamCoordinator` that owns a `TaskBoard`, a `MessageBus`, and per-agent `AgentWorker`s. Task dependencies use `$N` placeholders resolved to real IDs at creation time — that's how an LLM-generated plan can be handed to the coordinator without the LLM having to know task IDs it hasn't created yet. The run loop picks `get_ready_tasks()`, finds idle agents, and fires them in parallel with `asyncio.gather()`. Each agent gets its **own** `LLMClient(model)` instance, so intake can run on a cheap model while analysis runs on Sonnet. Tools are **role-gated**: `AGENT_TOOLS.get(role, ALL_TOOLS)` — least privilege so the intake agent can't accidentally call the BVA write path.

**Retrieval fan-out** ([`apps/api/app/agents/retrieval_agent.py`](https://github.com/va2ai/v2v-monorepo/blob/main/apps/api/app/agents/retrieval_agent.py)) is asymmetric because latency budgets aren't equal. BVA `search_extract` runs on the full query list with keyword-aware passage extraction. CFR and KnowVA run on only the **first two** queries. CAVC appellate precedent runs on the **first query only** — appellate lookups are the slowest and the marginal case usually isn't worth the wait. Keywords are extracted with a custom stopword filter (standard NLP stopword lists strip VA-specific short tokens like "SC" that actually carry signal).

**Qualitative eval with a real rubric.** Beyond the ablation table, I ran repeated end-to-end QA runs against the production URL scored on seven weighted dimensions ([`experiments/artifacts/assessments/qa-report-2.md`](https://github.com/va2ai/v2v-monorepo/blob/main/experiments/artifacts/assessments/qa-report-2.md)): Factual Accuracy (25%), Completeness (20%), Source Attribution (15%), Actionability (15%), UX/Responsiveness (10%), Tone/Empathy (10%), Safety/Disclaimers (5%). Run 1 scored 4.65/10, Run 2 scored 4.75/10 — honest "not production-ready" verdicts on both. The most useful finding: a run-2 regression where the pipeline misattributed TDIU's "substantially gainful activity" work restriction to 100% P&T schedular ratings (they're distinct categories; at 100% P&T schedular there are *zero* work restrictions). Root cause was pinned to the Analysis/Refinement agent handoff blending two regulatory categories. Fix: explicit prompt guardrails on the Analysis agent ("When discussing 100% P&T schedular ratings, never apply TDIU-specific restrictions"). **The value of the rubric wasn't the score — it was surfacing the class of error.**

**Theory Distiller — user-model research, not UI research.** The original research panel organized output by *source type* (BVA grants, CFR hooks, evidence types) — but attorneys think in *legal theories* ("service connection via nexus", "benefit of the doubt"), each drawing from CFR + BVA + M21 + evidence. The overhaul plan ([`experiments/artifacts/plans/theory-clusters-overhaul.md`](https://github.com/va2ai/v2v-monorepo/blob/main/experiments/artifacts/plans/theory-clusters-overhaul.md)) replaces the 5-panel grid with a `LegalTheory` cluster schema — each theory carries a hypothesis, a 0–1 strength score, CFR citations, key cases, evidence needed, the primary counter-risk to watch for, and a pre-built "dig deeper" query. The distiller runs async fire-and-forget after the final report (same pattern as the knowledge-base distiller). Getting the *primitives* right in research output matters more than the chart library choice.
...
**Validators are tests; failures are repair feedback.** v3 added an explicit generate-validate-repair gate around synthesis, with **two validators** in parallel because semantic support and citation grounding are genuinely different failure modes. The faithfulness critic decomposes the synthesized memo into atomic claims and checks each against the retrieved source context. The citation validator ([`src/services/research/agents/citation-validator.ts`](https://github.com/va2ai/va-claims-intel/blob/main/src/services/research/agents/citation-validator.ts)) extracts every 38 CFR / BVA / CAVC citation and checks it against the retrieved authority set with regex + normalization + set membership — *not* a model self-critique. The model can state a true legal proposition while attaching an unretrieved authority, and only the deterministic validator catches that. Failures from either validator become directive repair feedback (`replace with retrieved authority or remove`); a bounded retry loop runs up to 2 passes, and on retry exhaustion the output surfaces as `partially_verified` with explicit unresolved-issue metadata rather than degrading silently.

**Pre-registered evaluation, not vibes.** I wrote up the methodology as a paper before populating numbers. [`docs/legal-research-validation-loop-paper.pdf`](https://github.com/va2ai/va-claims-intel/blob/main/docs/legal-research-validation-loop-paper.pdf) is the case study of the architecture; [`va-claims-ai-paper/`](https://github.com/va2ai/va-claims-intel/tree/main/va-claims-ai-paper) is the longer evaluative paper pre-registering a six-metric rubric (citation fidelity, unsupported assertions, issue coverage, evidence-gap detection, legal traceability, reviewer usefulness) for double-blind comparison of direct prompting against the source-grounded pipeline. The paper's `tests/` directory enforces the spec mechanically: `evaluation_required.py` blocks any superiority claim until `data/scores.csv` is populated; `source_hierarchy.py` fails outputs that cite only BVA or M21-1 without a tier hedge; `no_legal_advice.py` and `overclaiming_check.py` block outcome-assurance and replace-counsel framing. The eval harness ([`scripts/run-va-claims-eval.ts`](https://github.com/va2ai/va-claims-intel/blob/main/scripts/run-va-claims-eval.ts)) runs four pipeline conditions (`direct_prompt`, `vanilla_rag`, `rag_self_critique`, `full_validation_loop`) across ten VA scenarios, blinds outputs, and scores them with two AI scorers at different temperatures — same harness scores my agent and the baseline against the same rubric. Smoke test on 1 scenario: 86 seconds end to end.

**Architectures I retired.** I had a "playbook architecture" spec on deck — `Playbook` interface, `DomainResolver`, `CorpusRegistry`, generic case-routing apparatus. After actually solving a research problem with just the V2V tool stack and a system prompt, I cut it. [`docs/agent-spec.md`](https://github.com/va2ai/va-claims-intel/blob/main/docs/agent-spec.md) replaces it with one server-side agent + the existing tools + a system prompt enforcing eight reasoning moves (frame → plan → retrieve → extract → compose → validate citations → validate faithfulness → repair-or-surface). Title 38 CFR isn't a "corpus" the agent has to register — it's just what `bva_cfr_section` returns when you call it. Knowing what *not* to build is part of the work; I wrote the spec down so future me can't forget why.

**Biggest dependencies.** Claude API pricing and rate limits. BVA source-format changes would ripple through retrieval. Observability is split across Firebase and Cloud Run, which slows incident response. The win I'm proudest of at the product level is splitting the UX into **Veteran mode** (simplified denial-analysis wizard) and **Professional mode** (full research workspace with CAVC Tracker, Precedent Finder, Research Orchestrator) — same backend, two surface areas. Audience fit beats feature parity. Pros hated hand-holding; veterans drowned in the firehose.
